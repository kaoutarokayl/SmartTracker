...
const Attendance = () => {
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      ...
      try {
        ...
        const filtered = timeRange === "24" ? data.filter(entry => entry.date === today) : data;
        setFilteredData(filtered);

        // Recalculer le résumé sur les données filtrées
  ...
  useEffect(() => {
    ...
    // Recalculer le résumé après filtrage par recherche
    const summary = filtered.reduce((acc, entry) => {
      const arrival = new Date(entry.arrival).toLocaleTimeString();
      const departure = new Date(entry.departure).toLocaleTimeString();
      const effectiveHours = calculateEffectiveHours(arrival, departure);
...
<input
  ...
  className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full mb-2"
/>
